...
# For each protocol/category go to their 'account' page, scrape data from each page of 'account' data, then add data to CSV file
# **kwargs: 'starting_point': name of the protocol/category to start scraping at
def scrape_escan_accounts(**kwargs):
    # Setup webdriver
    ##################################################################################################
    options = Options()
    ...
    # Get list of URLs to visit --> want the "account" and "token" URLs
    ##################################################################################################
    print("Make url lists for 'accounts'")
    ...
    try:
        ...
        after_start = True
    # get "account" href for each protocol/category
    # syntax for href: "/accounts/label/name-of-protocol"
    dropdowns = driver.find_elements("xpath", "//div[div/a[contains(@href, '/accounts/label/')]][@class='dropdown']")
    for x in dropdowns:
        link = x.find_element("xpath", ".//a[contains(@href, '/accounts/label')]")
    ...
    for protocol in base_url_list:
        ...
        ##############################################################################################
        url = f"{base_url}?size={page_size}&start={start_pg_index}&col=1&order=asc"
        ...
        for x_sub in subcat_list:
            ...
            while end_of_pages is False:
                if x_sub == subcat_list[0] and start_pg_index == 0:
                    ...
                    url = f"{base_url}?subcatid={x_sub['subcat_val']}&size={page_size}&start={start_pg_index}&col=1&order=asc"
                    driver.get(url)
                    print(f"Go to url: {url}")
                    time.sleep(5)
                ...
                print(f"on last page: {current_page == last_page}")
                ...
                for i in range(1, table_rows + 1):
                    try:
                        address = driver.find_element(
                            "xpath", f"//*[@id='table-subcatid-{x_sub['subcat_val']}']/tbody/tr[{i}]/td[1]//a"
                        ).text
                    ...
                    try:
                        name_tag = driver.find_element(
                            "xpath", f"//*[@id='table-subcatid-{x_sub['subcat_val']}']/tbody/tr[{i}]/td[2]"
                        ).text
                        name_tag = emoji.replace_emoji(name_tag, "")  # if there's an emoji in the name then replace it
                    except NoSuchElementException:
                ...
                if first_loop is True:
                    write_csv("eth_trader_profit/scrapers/csv_files_web", "etherscan_account_scrape_2", data_scraped)
                    first_loop = False
                else:
                    append_csv("eth_trader_profit/scrapers/csv_files_web", "etherscan_account_scrape_2", data_scraped)
                time.sleep(5)
                data_scraped = []
...
def main():
    scrape_escan_accounts(starting_point="Beeple")
    # scrape_escan_accounts()
